...
from newnucal.multiresolution import init_resampled_joint_state
...
def _run_state(cal, state, n_iter):
    t0 = time.perf_counter()
    state = cal.run_joint_sky_beam_dirty_state(state, n_iter=n_iter, verbose=False)
...
def main(args):
    ...
    low_state, low_time = _run_state(cal_low, low_state, args.low_iter)
    ...
    high_state, refine_time = _run_state(cal_high, high_state, args.high_iter)
    ...
    direct_state, direct_time = _run_state(
        cal_high, direct_state, args.low_iter + args.high_iter
    )
